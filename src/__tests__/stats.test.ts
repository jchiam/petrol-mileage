...
function makeFill(
  overrides: Partial<FillUp> & { id?: number; pumpDate?: string; petrolL?: string; mileageKm?: string; cost?: string },
): FillUp {
  return {
    id: overrides.id ?? 1,
    vehicleId: 1,
...
describe('detectAnomalies', () => {
  ...
  it('flags price anomaly when $/L is >15% above median', () => {
    ...
    const flags = detectAnomalies({ kmPerL: 12, costPerKm: 0.18, costPerL: 2.50 }, goodTrailing)
  ...
  it('does not flag price when $/L is exactly at 15% (boundary)', () => {
    ...
    const flags = detectAnomalies({ kmPerL: 12, costPerKm: 0.16, costPerL: 2.30 }, goodTrailing)
    expect(flags.find((f) => f.type === 'price')).toBeUndefined()
  })

  it('does not flag price when $/L is below median', () => {
    const flags = detectAnomalies({ kmPerL: 12, costPerKm: 0.16, costPerL: 1.50 }, goodTrailing)
    expect(flags.find((f) => f.type === 'price')).toBeUndefined()
  })
})
...
describe('computeKPIs', () => {
  ...
  it('excludes voided fills from KPI computation', () => {
    ...
    const fills: FillUp[] = [
      makeFill({ id: 1, pumpDate: today, petrolL: '40', mileageKm: '500', cost: '80', voidedAt: new Date() }),
...
describe('computeForecast', () => {
  ...
  it('nextMonthLow <= nextMonthExpected <= nextMonthHigh', () => {
    ...
    const fills: FillUp[] = Array.from({ length: 10 }, (_, i) =>
      makeFill({
        id: i + 1,
        pumpDate: new Date(Date.now() - (i + 1) * 8 * 86_400_000)
  ...
  it('annualProjection >= ytdSpend', () => {
    ...
    const fills: FillUp[] = Array.from({ length: 5 }, (_, i) =>
      makeFill({
        id: i + 1,
        pumpDate: new Date(Date.now() - (i + 1) * 15 * 86_400_000)
          .toLocaleDateString('en-CA', { timeZone: 'Asia/Singapore' }),
        cost: '80',
      }),
    )
