...
export function computeMetrics(fill: Pick<FillUp, 'petrolL' | 'mileageKm' | 'cost'>): FillMetrics {
 ...
 */
export function detectAnomalies(current: FillMetrics, trailing: FillMetrics[]): AnomalyFlag[] {
...
export function computeCharts(fills: FillUp[]): ChartData {
  ...
  const fillPoints: ChartFillPoint[] = active.map((f, i) => {
    ...
    const rolling5 =
      window.length >= 2
        ? Math.round(mean(window.map((w) => computeMetrics(w).kmPerL)) * 100) / 100
        : null;
    return {
      date: f.pumpDate,
      id: f.id,
  ...
  const fillsWithAnomalies: FillWithAnomalies[] = sorted.map((fill) => {
    ...
    if (!fill.voidedAt) {
      ...
      const trailing = active
        .filter(
          (f) => f.pumpDate < fill.pumpDate || (f.pumpDate === fill.pumpDate && f.id < fill.id),
        )
        .slice(-10)
        .map(computeMetrics);
